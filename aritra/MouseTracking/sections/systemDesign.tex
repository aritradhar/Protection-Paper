...
\myparagraph{Setup} We assume that the \device manufacturer issues a certificate for each of the deployed \device{}s . The \device maintains a whitelist for the remote servers along with their public certificates. This allows the \device to verify messages signed by those remote servers. It is also possible to establish a conventional \tls channel between the \device and the server using the HDMI signal as downstream channel and keystrokes from the \device as the upload channel. Note that IO integrity does not require a \tls between the \device and the remote server. Such a \tls channel is strictly necessary to provide IO confidentiality that is discussed later in Section~\ref{sec:confidentiality}.  For now, we only focus on IO integrity in this section.\red{Here let's focus on what we do, not why we need TLS but don't use until next section.}
...
Protecting the integrity of the mouse actions (move, drag, and click), also known as the \emph{pointer integrity} is a non-trivial task as one needs to know the user's context (such as the location of the mouse pointer and the corresponding UI elements that the user interacts with) on the screen. \red{Host's pointer cannot be visible when the user interacts with the overlay rendered by the \device. Pointer tracking by the device is not easy because we do not trust the host to communicate to the \device pointer's correct position.} \red{The attacker is in control of the host and he can easily compromise non-protected elements, but he cannot access UI elements overlaid by the \device.} Moreover, the attacker can execute clickjacking attacks~\cite{huang2012clickjacking} by fooling the user into \red{clicking on sensitive UI elements while}. %following another malicious mouse pointer in other position.

\name's pointer tracking \red{(integrity?)} mechanism ensures that i) the user always follows the correct mouse pointer and does not get tricked into clicking unintentionally a security-critical UI element (\red{is it more accurate if we say: the \device and the user have the same sense of pointer position}), and ii) all mouse activities (movements and clicks) originate from the user. The \device intercepts the HDMI frames and mouse signals to: 1) extract the position of the mouse pointer and correlate it with the mouse data and 2) overlay a mouse pointer that \red{is prominent and} serves as a visual cue for the user.

\subsubsection{\bfseries Calibration}\label{sec:systemDesign:analysis:calibration} When the user connects the \device for the first time after booting up, the \device performs an automated calibration to find the pointer. The \device gives very high mouse movement values that push the mouse pointer to the top-right corner of the screen. Then the \device tries to find the pointer there from the HDMI frames. If the \device is successful into finding the mouse pointer there, it continues tracking the pointer after that. Note, that at any point, if the \device loses the track of the mouse pointer, the user can recalibrate it by reinitializing the \device.

\subsubsection{\bfseries Detection of pointer} Figure~\ref{fig:mouseAnalysis} illustrates the high-level idea of the host system's HDMI frame analysis. To match the mouse polling rate with the display frame rate, the \device only queries the input device with the frequency of $30$ Hz. We assume that over the HDMI channel the host system sends frames at the rate of $30$ fps. Our detection of the mouse pointer does not require to analyze the entire resolution of the HDMI frame, but rather a small area of the frame which is efficient. We define the mouse trace as the time series $(\Delta x_i = |x_{t_{i-1}} - x_{t_{i}}|, \Delta y_i=|y_{t_{i-1}} - y_{t_{i}}|)$ delta co-ordinates: $\{(\Delta x_1, \Delta y_1), (\Delta x_2, \Delta y_2), \ldots, (\Delta x_n, \Delta y_n)\}$ from time $\{t_1, t_2, \ldots, t_n\}$. $x_{t_{i}}$ and $y_{t_{i}}$ denote the pixel position of the mouse pointer on the screen at time $t_i$. At the time of initialization, i.e., the first time the \device turns on after the host boots up, the \device pushes the mouse at the left-upper corner of the screen, hence the initial mouse pointer position is set to $(x_0, y_0) = (0, 0)$.  
Note that a mouse only provides displacement of over $x$ and $y$ coordinates. This corresponds that at time $t_i$ the mouse reported $(\Delta x_i, \Delta y_i)$ displacement to the \device. Assume that the frames coming from the host system to the \device are: $\{f_1, f_2, \ldots, f_n\}$ over the same time interval $(t_1, t_2,\ldots t_n)$. At the time $t_i$, the \device looks into the frame $f_i$ and draws a square centered at $(x_i, y_i)$ with sides of length $X$ (which is enough to cover a mouse cursor on the screen\footnote{In our evaluation, we notice that oa 50 $\times$ 50 square pixels can cover the default mouse pointer provided by Ubuntu.}). Then the \device checks if there exists a mouse inside this square or not. In case there exists a mouse cursor, the \device allows further user interactions; otherwise, it stops all the communications and shows an error on display. The implementation detail is provided in Section~\ref{sec:prototype:impl:mouse}.\red{This description is mainly technical}
...
%\red{Do we need to \texttt{overlay} the cursor always or only when \name is enabled. We track it always though.}
...
\red{This does not give any info here, can be removed. }
