...
Protecting the integrity of the mouse actions (move, drag, and click), also known as the \emph{pointer integrity} is a non-trivial task as one needs to know the user's context (such as the location of the mouse pointer and the corresponding UI elements that the user interacts with) on the screen. Host's pointer cannot be visible when the user interacts with the overlay rendered by the \device. Pointer tracking by the device is not easy because we do not trust the host to communicate to the \device pointer's correct position. The attacker is in control of the host and he can easily compromise non-protected elements, but he cannot access UI elements overlaid by the \device. Moreover, the attacker can execute clickjacking~\cite{huang2012clickjacking} attacks by fooling the user into tracking a malicious mouse pointer. \name's pointer tracking mechanism ensures that i) the user always follows the correct mouse pointer and does not get tricked into clicking unintentionally a security-critical UI element on the screen i.e., the \device and the user have the same sense of pointer position, and ii) all the mouse activities are originated from the user. The \device and intercepts the HDMI frames and mouse signals to: 1) extract the mouse pointer and correlate it with the mouse data and 2) overlay a mouse pointer that is prominent and easy to follow.
