...
The major functionalities of the JavaScript in the context of input fields are i) input validation, and ii) user feedback animations. As \name targets to protect the integrity of safety-critical UIs, we decide to preserve only the functional aspect of the JS, i.e., input validation. The remote server sends the regular expression in the UI specification (\texttt{RE} in Specification~\ref{snippet:UISpecification}) that the \device uses to validate the user input.
}
