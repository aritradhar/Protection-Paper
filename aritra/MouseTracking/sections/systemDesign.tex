...
Protecting the integrity of the mouse actions (move, drag, and click), also known as the \emph{pointer integrity} is non-trivial as one needs to know the user's context (such as the location of the mouse pointer and the corresponding UI elements that the user interacts with) on the screen. Moreover, the attacker can execute clickjacking~\cite{huang2012clickjacking} attacks by fooling the user into tracking a malicious mouse pointer. \name's pointer tracking mechanism ensures that i) the user always follows the correct mouse pointer and does not get tricked into clicking unintentionally a security-critical UI element on the screen, and ii) all the mouse activities are originated from the user. The \device and intercepts the HDMI frames and mouse signals to: 1) extract the mouse pointer and correlate it with the mouse data and 2) overlay a mouse pointer that serves as a visual cue for the user.
