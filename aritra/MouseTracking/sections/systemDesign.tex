...
\subsubsection{Coping with the blinking cursor} In the case the user is typing in a text editor, in some OS (e.g., Windows), the cursor disappears from the screen.  When the user moves her mouse again, the cursor again appears. To handle such a scenario, the \device listens to all the keyboard input as the keyboard is also connected to the \device. When the \device detects there are keystrokes, it expects the cursor to be disappeared from the last detected position. When the \device receives input from the mouse again, it checks if the cursor appears again on the screen at the exact location where it disappeared - this way the \device ensures the consistency of the pointer position.  
...
<form action="/some_action">
  ...
  <encrypted>
  0x4a5c4... <!-encrypted UI specification->
  </encrypted>
  <script>
