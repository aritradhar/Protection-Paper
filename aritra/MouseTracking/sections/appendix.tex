...
In this section, we provide a formal proof for the security properties of \name. Namely,  without protecting both input and output integrity, none of them can be achieved. To simplify the proof, we model the interaction between the user, the host, and the remote server as a finite state automaton (FSA).
