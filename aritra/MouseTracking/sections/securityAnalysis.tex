...
In this section, we analyze the security of \name by modeling the interaction between the user, host and the remote server. We assume that the user behaves reasonably and we argue that \name provides sufficient mechanism to isolate the trusted part of the screen (\device overlay) from the untrusted part of the screen. As long as the user does not get influenced by the information provided in the untrusted part of the screen and modifies her input, the \name remains secure.
