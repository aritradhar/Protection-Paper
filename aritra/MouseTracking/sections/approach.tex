...
\subsubsection{\bfseries Strawman solution IV}
\label{sec:approach:strawman:4}
...
\myparagraph{Key idea} %\red{The key idea of \name is to introduce \emph{root-of-trust} for the IO devices that provide a trusted path to remote servers. This is achieved by the \device that provide proof for integrity and confidentiality for all user IO. Thus making the IO devices trustworthy to the users. }
The strawman solutions in the previous section provides the intuition for the design of our solution \name. All the exiting trusted path solution failed to achieve input integrity as it can not be achieved without the output integrity. Output integrity ensures that the user receives all the information from the remote server on her output devices as it was originally intended by the server. Moreover, the host should not have the privilege to submit the data of a form because that would allow the malicious host to submit a incomplete input data that may results in misconfiguration. The design of \name is build upon this idea of ensuring the output integrity.   
...
%A naive solution with this system configuration would be to run the entire OS and browser on the \device to provide isolation from the attacker-controlled host. Such an approach makes the size of the \device TCB massive, making it as vulnerable as the host.\red{Will this be included in the strawman solutions?}
