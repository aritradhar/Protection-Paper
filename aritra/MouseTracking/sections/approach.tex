...
As the device also sits on the HDMI channel between the host and display device, it can overlay objects on the HDMI stream and thus achieves display integrity and privacy. All the objects that are overlaid by the device can only be seen by the user and completely oblivious to the host. Using these properties, the \device ensures that the user is viewing the legitimate verion of the UI objects, thus provide \emph{\poui}.
