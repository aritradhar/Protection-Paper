...
\blue{There are several possible ways to deploy the \name system. Here we outline two example deployments. The first example deployment is one where a service provider like a bank issues a \device device to each of its customers. In such a deployment, the \device is intended to be used with a single application like web-based online banking and it is pre-configured with the public key certificate of that application server (e.g., online banking server certificate). Such pre-installed certificates allow the \device to verify messages signed by the correct application server. The service provider can ask what OS the customer uses and configure OS-specific settings like the used SAS value to the issued device (see Section~\ref{sec:confidentiality:SAS} for details).} 
...
\blue{In another example deployment, the \device device is issued by a third-party vendor and it is intended to be used with various security-critical online services. In such a deployment, the \device device can be pre-configured with the public key of its issuer and a white-list of trusted application server certificates. The issuer of the device can issue authenticated updates to the white-list after its deployment if needed.}
