...
For every request, the server adds a random identifier (\texttt{id}) to the specification file, signs it, and then delivers it to the user's browser. The \texttt{id} serves as session identification to prevent the attacker from re-submitting an old input data drom the user.
