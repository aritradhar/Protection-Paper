...
\subsubsection{\bfseries Entering/exiting secure mode with keyboard} \red{(\#6)In our implementation of \name, entering and exiting the mouse pointer invokes the secure mode. But similar could be done by the keyboard also. The user could use the \texttt{TAB} button that selects the QR code on the browser. In the \name JS, we can define properties that trigger some changes (by changing the selection parameter) into the QR specification that signals the \device to enter/exit the secure UI.}
