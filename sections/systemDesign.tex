...
One of the widely used JavaScript functionalities in the context of input from is input validation. Input validation script is executed at the client-side that checks the input against a recommended input policy (e.g., regular expression). Input validation is also a critical part of input integrity. The remote server sends the regular expression in the UI specification (\texttt{RE} in Specification~\ref{snippet:UISpecification}) that the \device uses to validate the user input.
}
