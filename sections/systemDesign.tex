...
\myparagraph{(i) Secure form $\rightarrow$ Specification}
Developers should manually mark the sensitive elements of a form, and then produce the respective specification.
%uses a small code that transforms the HTML forms (only the forms with elements with attribute \texttt{protect=``true''}) to the JSON specification and signs it. 
For every request, the server adds a random identifier (\texttt{id}) to the specification file, signs it, and then delivers it to the user's browser. The \texttt{id} serves as session identification to prevent the attacker from re-submitting an old input data from the user.
...
%The \device detects the encoded specification, reads it and then renders the overlay. Note that the \device renders only specifications that are signed by the trusted server. Therefore the attacker cannot alter it. 
...
Lightbox mitigates the attacks presented above. \red{the authors~\cite{huang2012clickjacking} also show that the lightbox is effective $98\%$ of the time while the freezing is effective in $97\%$ time (baseline: $69\%$ effectiveness when no protection is provided). For more details of the user study, refer to Table 2 in~\cite{huang2012clickjacking}, making them suitable candidates for \name. We assume that similar result should be expected in \name due to the similarity of the application space (web applications).} \device uses Lightbox as the default technique, but depending on the specific form, the developers can select the appropriate technique.
...
%In the previous section, we discuss how to focus user attention to the part of the screen overlaid by \device. The focusing mechanisms rely on the user's mouse movement, which should trigger the focusing mechanism when entering the overlay area. 
...
\red{\myparagraph{Input validation}
One of the widely used JavaScript functionalities in the context of input form is input validation. Input validation script is executed at the client-side that checks the input against a recommended input policy (e.g., regular expression). Input validation is also a critical part of input integrity. The remote server sends the regular expression in the UI specification (\texttt{RE} in Specification~\ref{snippet:UISpecification}) that the \device uses to validate the user input.
}
...
\red{\myparagraph{Downgrading for legacy clients} \name is backward-compatible with the clients who do not use the \device. This is achieved by the remote server by showing a QR code briefly on the screen when the user visits the webpage. The \device intercepted the QR code and sends a signal to the server about its presence. In the absence of the \device, the remote server does not send the \name JS to the host that acts as a communication channel between the \device and the remote server. Note, that this fallback mechanism is application-specific and the service provider could decide if the fallback is detrimental to security.
}