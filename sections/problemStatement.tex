...
\myparagraph{B3.~System TEE-based solutions} \red{Solutions such as VButtion~\cite{li2018vbutton} uses ARM TrustZone (TZ) to securely render buttons that are isolated from the OS. TZ uses flags on the bus to isolate sensitive data from the untrusted rich OS. However, system TEEs such as TZ is infeasible for us for the following reasons i) TZ is not available for x86 architecture -- similar system TEE in x86 requires changes in the existing architecture, ii) TZ requires specific TZ-aware applications -- general web based UIs requires browser modification which could be infeasible due to their massive code-base (see Table~\ref{tab:loc}).  }    
