...
\resizebox{\textwidth}{!}{
  \begin{tabular}{l | l | c  c  c  c | c  c  c  c | c c} 
    ...
    & & \multicolumn{4}{c}{} & \multicolumn{3}{c}{\cellcolor{Gray}\textbf{R2}} & \multicolumn{3}{c}{} \\
    & & \multicolumn{8}{c}{} & \multicolumn{2}{c}{\cellcolor{Gray}\textbf{R3a/b}} \\
    & & \multicolumn{4}{c}{\cellcolor{Gray}\textbf{R4}} & \multicolumn{6}{c}{} \\ \hline
   \multirow{4}{*}{Category}
   ...
   \hline
    &Browser-based~\cite{ye2005trusted}			 &  		&   	& \yes 		& \yes 	&  	\no		& \no 	& \no 		& \yesNope & \no &\yes\\
    \rowcolor{Gray}
   	\cellcolor{white} & InContext~\cite{Overshadow} 				 &  		&  	&  	  	& \yes 	& \no 			& \yes 	& \no 		& \no & \no  &\yes\\
    ...
    &Splitting interfaces~\cite{ta2006splitting}  &  		&  	&  		& \yes 	& \yes 			& \no 	& \no 		& \yes &\no &\no\\
    \rowcolor{Gray}
    \cellcolor{white}&$SP^3$~\cite{yang2008using} 				 &  		&  	&  		& \yes 	& \yes 			& \no 	& \no 		& \no &\no &\no\\
    &SGX IO~\cite{weiser2017sgxio}  				 & \yes 	&  	& \yes 	& \yes	& \yes 			& \no 	& \no 		& \no &\no &\no\\
    \rowcolor{Gray}
    \cellcolor{white}\parbox[t]{1mm}{\multirow{-11}{*}{\rotatebox[origin=c]{90}{\textbf{Hypervisor/OS-based}}}}  \ldelim\{{-10}{0mm}[] & SchrodinText~\cite{sani2017schrodintext}	 & \yes 	&   &  	& \yes 	& \no 			& \no 	& \no 		& \yes &\no & \no\\
    &BASTION-SGX~\cite{BASTION-SGX}			     & \yes 	&   	&  		&  	& \yes 			& \no 	& \no 		& \no &\no &\yes\\
    \rowcolor{Gray}
    \cellcolor{white}&Slice~\cite{azab2011sice}				     & \yesNope &   	&  		&  	& \no 			& \no 	& \no 		& \no &\no &\no\\
    &TrustOTP~\cite{sun2015trustotp}			     & \yes 	&   	&  		&  	& \yes		 	& \no 	& \no 		& \yesNope &\no &\yes\\
    \rowcolor{Gray}
    \cellcolor{white}&VeriUI~\cite{liu2014veriui}				     & \yes 	&   & \yes 		&  	& \yesNope 		& \no 	& \no 		& \yesNope &\no &\no\\
	&AdAttester~\cite{li2015adattester}			 & \yes 	&   & \yes 		&  	& \no 			& \no & \yesNope 	& \yesNope &\no &\no\\
	\rowcolor{Gray}
	\cellcolor{white}&TruZ-Droid~\cite{ying2018truz}			     & \yes 	&   & \yes 		&  	& \yes 			& \no 	& \no 		& \yesNope &\no &\yes\\
	&TrustUI~\cite{li2014building}			     & \yes 	&   & \yesNope 	&  	& \no 			& \no 	& \yesNope 		& \yesNope &\no &\yes\\
	\rowcolor{Gray}
	\cellcolor{white}&VButton~\cite{li2018vbutton}			     & \yes 	&   & \yes 	&  	& \yesNope 			& \no 	& \yes 		& \yes &\no &\no\\
    &CARMA~\cite{vasudevan2012carma}			     & \yes 	& \yes 	&  		&  	& \no 			& \no 	& \no 		& \no & \yes &\no\\
    \rowcolor{Gray}
    \cellcolor{white}&\textsc{ProximiTee}~\cite{dhar2018proximitee}&\yes 		& \yes  & \yesNope 	&  	& \yes 			& \no 	& \no 		& \no &\yes &\yes\\
     \cellcolor{white}\parbox[t]{3mm}{\multirow{-13}{*}{\rotatebox[origin=c]{90}{\textbf{TEE-based}}}}  \ldelim\{{-13}{0mm}[] & Fidelius~\cite{Fidelius}			   	     & \yes 	& \yes  & \yes 		&  	& \yes 			& \no 	& \no 		& \yesNope & \no & \no\\
    \rowcolor{Gray}
    \cellcolor{white}&FPGA-based~\cite{brandon2017trusted}		 &  		& \yes  &  		&  	& \yes 			& \no 	& \no 		& \yes & \no &\no\\
    &IntegriKey~\cite{IntegriKey}				 &  		& \yes  & \yesNope 	&  	& \yesNope 		& \no 	& \no 		& \no & \yes &\yes\\ 
    \rowcolor{Gray}
    ...
    \hline
    \multicolumn{10}{c}{\small\yes~requires/supports \hspace{1cm} \no~ does not support \hspace{1cm} \yesNope ~partially requires/supports}  
  \end{tabular}
  }
