...
\parasave
\red{(\#10)\myparagraph{Fake SAS} The attacker may put a fake SAS message on the screen tricking the user into reveling her sensitive information to the attacker. This attack is not possible as \name follows a standard, system-wide SAS sequence that is consistent across different web servers (such as \texttt{Ctr+Alt+Del}). Recall that the SAS key stroke is first intercepted by the \device. SAS is expected to be used when there exists a QR code on the screen. In case there is no QR code on the screen, the \device warns the user.}
