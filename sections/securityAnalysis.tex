...
\red{\myparagraph{Malicious QR codes} The attacker may put fake QR codes on the webpage. But note that the \device verifies the signature from the HTML forms to check the integrity. This way, the \device does not render any overlays from malicious QR codes.} 
...
\red{\myparagraph{Fake SAS} The attacker may put a fake SAS message on the screen tricking the user into reveling her sensitive information to the attacker. This attack is not possible as \name follows a standard SAS that is consistent across different web servers. The SAS key stroke is first intercepted by the \device. If there exists a QR code on the screen. In case there is no QR code on the screen, the \device warns the user.}
